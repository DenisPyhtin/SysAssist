...
public sealed class SysAssistServiceTests
{
    ...
    [Fact]
    public async Task ExecuteAction_WhenApprovalIsRequired_CreatesPendingApprovalRequest()
    {
        var service = CreateService();
        await service.RunDiagnosticsAsync("test", "test-diagnostics", CancellationToken.None);
        var module = await ModuleByKey(service, "local-rule-advisor");
        var action = (await service.ListActionsAsync(CancellationToken.None))
            .Single(item => item.ModuleId == module.Id && item.ActionKey == "raise_confidence_threshold");
        var beforeApprovals = await service.ListApprovalsAsync(CancellationToken.None);
        var beforeEvents = await service.ListEventsAsync(CancellationToken.None);

        var result = await service.ExecuteActionAsync(action.Id, new ExecuteActionRequest("advisor-rules", """{"threshold":"0.85"}"""), "engineer", "test-action-approval", CancellationToken.None);

        var afterApprovals = await service.ListApprovalsAsync(CancellationToken.None);
        var approval = afterApprovals.Single(item => beforeApprovals.All(previous => previous.Id != item.Id));
        var incident = await service.GetEventAsync(approval.EventId, CancellationToken.None);
        var afterEvents = await service.ListEventsAsync(CancellationToken.None);

        Assert.True(result.Success);
        Assert.Contains("Approval request created", result.Message, StringComparison.OrdinalIgnoreCase);
        Assert.Equal(beforeApprovals.Count + 1, afterApprovals.Count);
        Assert.Equal(beforeEvents.Count + 1, afterEvents.Count);
        Assert.Equal(action.Id, approval.ActionId);
        Assert.Equal("Pending", approval.Status);
        Assert.NotNull(incident);
        Assert.Equal("PendingApproval", incident!.Status);
        Assert.Equal("manual.action.requested", incident.EventType);
        Assert.Equal("advisor-rules", incident.Target);
    }

    [Fact]
    public async Task TriggerSabotage_CreatesSharedWorkflowIncident_AndPendingApprovalWhenExpected()
    {
        var service = CreateService();
        var module = await ModuleByKey(service, "linux-host");
        await service.SetModuleEnabledAsync(module.Id, true, "test", "test-sabotage-enable", CancellationToken.None);
        var beforeEvents = await service.ListEventsAsync(CancellationToken.None);
        var beforeApprovals = await service.ListApprovalsAsync(CancellationToken.None);

        var result = await service.TriggerSabotageAsync("linux-cpu-spike", "engineer", "test-sabotage", CancellationToken.None);

        var afterEvents = await service.ListEventsAsync(CancellationToken.None);
        var sabotageEvent = afterEvents.Single(item => beforeEvents.All(previous => previous.Id != item.Id));
        var afterApprovals = await service.ListApprovalsAsync(CancellationToken.None);
        var audit = await service.ListAuditAsync(CancellationToken.None);

        Assert.True(result.Success);
        Assert.Contains("Sabotage scenario triggered", result.Message, StringComparison.OrdinalIgnoreCase);
        Assert.StartsWith("sabotage-linux-cpu-spike", sabotageEvent.ExternalEventId, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[Sabotage]", sabotageEvent.Summary, StringComparison.Ordinal);
        Assert.Equal("PendingApproval", sabotageEvent.Status);
        Assert.True(afterApprovals.Count > beforeApprovals.Count);
        Assert.Contains(audit, item => item.Action == "SABOTAGE_TRIGGERED" && item.Resource == "linux-cpu-spike");
    }
