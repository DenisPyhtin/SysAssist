    ...
    IConfiguration? configuration = null) : ISysAssistApiService
{
    ...
    public async Task<OperationResultDto> ExecuteActionAsync(Guid id, ExecuteActionRequest request, string actor, string? correlationId, CancellationToken cancellationToken)
    {
        ...
        {
            return await QueueManualActionApprovalAsync(module, action, request, actor, correlationId, cancellationToken);
        }
    ...
    public Task<IReadOnlyCollection<SabotageScenarioDto>> ListSabotageScenariosAsync(CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        return Task.FromResult<IReadOnlyCollection<SabotageScenarioDto>>(SabotageCatalog.All);
    }

    public async Task<OperationResultDto> TriggerSabotageAsync(string key, string actor, string? correlationId, CancellationToken cancellationToken)
    {
        var scenario = SabotageCatalog.Find(key);
        if (scenario is null)
        {
            return NotFound("Sabotage scenario not found.", correlationId);
        }

        var module = dbContext is null
            ? DemoStore.Modules.SingleOrDefault(item => item.Key.Equals(scenario.ModuleKey, StringComparison.OrdinalIgnoreCase))
            : await dbContext.IntegrationModules.SingleOrDefaultAsync(item => item.Key == scenario.ModuleKey, cancellationToken);
        if (module is null)
        {
            return new OperationResultDto(false, $"Module {scenario.ModuleKey} was not found.", correlationId);
        }

        if (scenario.RequiresEnabledModule && !module.IsEnabled)
        {
            return new OperationResultDto(false, $"Enable module {module.Name} before running sabotage checks.", correlationId);
        }
        ...
        var incident = await ProcessIncomingEventAsync(
            module,
            new IncomingEventDto(
                module.Key,
                $"sabotage-{scenario.Key}-{Guid.CreateVersion7()}",
                scenario.EventType,
                ParseSeverity(scenario.Severity),
                scenario.Target,
                scenario.Summary,
                payloadJson,
                correlationId),
        ...
        await AuditAsync(actor, "SABOTAGE_TRIGGERED", scenario.Key, incident.Status.ToString(), correlationId, cancellationToken);
        await WriteSystemLogAsync("Warning", "Sabotage", $"Sabotage scenario {scenario.Name} triggered.", correlationId, $$"""{"scenario":"{{scenario.Key}}","module":"{{module.Key}}","eventId":"{{incident.Id}}"}""", cancellationToken);
        await SaveAsync(cancellationToken);
        return Ok($"Sabotage scenario triggered: {scenario.Name}.", correlationId);
    }
    ...
    private async Task<OperationResultDto> QueueManualActionApprovalAsync(IntegrationModule module, IntegrationModuleAction action, ExecuteActionRequest request, string actor, string? correlationId, CancellationToken cancellationToken)
    {
        var target = string.IsNullOrWhiteSpace(request.Target) ? module.Name : request.Target.Trim();
        var incident = new IncidentEvent
        {
            Source = module.Key,
            EventType = "manual.action.requested",
            Severity = EventSeverityFromRisk(action.RiskLevel),
            Target = target,
            Status = EventStatus.PendingApproval,
            CorrelationId = correlationId,
            Summary = $"Manual action requested: {action.Name}",
            PayloadJson = string.IsNullOrWhiteSpace(request.ParametersJson)
                ? $$"""{"origin":"actions-page","action":"{{action.ActionKey}}","module":"{{module.Key}}","target":"{{target}}"}"""
        ...
        {
            EventId = incident.Id,
            Classification = "manual.action.requested",
            Explanation = $"Operator requested {action.Name} from the Actions page.",
            Confidence = 1m,
            ProbableCause = module.Name,
            NextStep = "Await approval and execute the requested action.",
            SuggestedActionId = action.Id
        };
        AddRecommendation(recommendation);
        incident.RecommendationId = recommendation.Id;
        ...
        AddNotification("local", "SeniorAdmin", "Approval required", $"{action.Name} was requested for {target}.", NotificationStatus.LocalOnly, incident.Id);
        await AuditAsync(actor, "APPROVAL_REQUESTED", approval.Id.ToString(), "Pending", correlationId, cancellationToken);
        await WriteSystemLogAsync("Warning", "Actions", "Action approval request created.", correlationId, $$"""{"action":"{{action.ActionKey}}","module":"{{module.Key}}","eventId":"{{incident.Id}}","approvalId":"{{approval.Id}}"}""", cancellationToken);
        await SaveAsync(cancellationToken);
        return Ok($"Approval request created for {action.Name}.", correlationId);
    }
    ...
    private static EventSeverity EventSeverityFromRisk(RiskLevel riskLevel) =>
        riskLevel switch
        {
    ...
    private static EventSeverity ParseSeverity(string severity) =>
        Enum.TryParse<EventSeverity>(severity, ignoreCase: true, out var value)
            ? value
            : EventSeverity.Warning;
...
internal static class SabotageCatalog
{
    public static readonly IReadOnlyCollection<SabotageScenarioDto> All =
    [
        new("linux-cpu-spike", "Linux CPU saturation", "Push a critical host-pressure incident through the alert workflow and approval queue.", "linux-host", "Critical", "linux.cpu.saturation", "app-api-02", "[Sabotage] Sustained CPU saturation on app-api-02", true, true),
        new("redis-memory-pressure", "Redis memory pressure", "Simulate cache saturation and validate operator-facing warning flow.", "redis", "Warning", "redis.memory.pressure", "cache-primary", "[Sabotage] Redis memory usage exceeded the configured threshold", true, false),
        new("postgres-lock-storm", "PostgreSQL lock storm", "Create a database contention event that should recommend diagnostics and likely remediation.", "postgresql", "Error", "postgres.lock.contention", "db-primary", "[Sabotage] Long transaction lock storm detected on db-primary", true, true),
        new("nginx-502-wave", "Nginx 502 wave", "Inject a gateway failure pattern for ingress alert validation.", "nginx", "Error", "nginx.gateway.502", "edge-nginx-01", "[Sabotage] Repeated 502 responses detected on edge-nginx-01", true, true),
        new("alert-route-drift", "Alert route drift", "Exercise Alertmanager routing and approval pressure from the alerting tier.", "prometheus-alertmanager", "Critical", "alertmanager.route.drift", "alert-routing", "[Sabotage] Alert route drift detected in Alertmanager", true, true),
        new("http-probe-flap", "HTTP probe flap", "Generate an intermittent endpoint failure without escalating to critical approval flow.", "http-endpoint", "Warning", "http.probe.flap", "checkout-api", "[Sabotage] Endpoint probe shows intermittent failures on checkout-api", true, false)
    ];

    public static SabotageScenarioDto? Find(string key) =>
        All.SingleOrDefault(item => item.Key.Equals(key, StringComparison.OrdinalIgnoreCase));
}
