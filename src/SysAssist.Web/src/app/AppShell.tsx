...
const navItems: Array<{ id: PageId; label: string; icon: typeof Gauge; roles?: string[] }> = [
  ...
  { id: 'sabotage', label: 'Sabotage', icon: AlertTriangle, roles: ['Engineer', 'Admin', 'SeniorAdmin'] },
]
...
function invalidateOperationalQueries(queryClient: QueryClient) {
...
function ApprovalsPage() {
  ...
  const pendingApprovals = data.filter((item) => item.status === 'Pending')
  const selectedApproval = pendingApprovals.find((item) => item.id === selectedApprovalId) ?? pendingApprovals[0]
        ...
        <div className="space-y-4">
          ...
          <Panel>
            ...
            <div className="space-y-3">
              ...
              {pendingApprovals.length === 0 ? <EmptyState icon={ClipboardCheck} title="No approval requests" /> : null}
            </div>
          </Panel>
        </div>
        <Panel>
...
function isSabotageEvent(event: EventDto) {
  return event.externalEventId?.startsWith('sabotage-', 0) || event.payloadJson?.includes('"sabotage":true')
}

function SabotagePage() {
  const queryClient = useQueryClient()
  const pushToast = useShellStore((state) => state.pushToast)
  const openEvent = useShellStore((state) => state.openEvent)
  const setActivePage = useShellStore((state) => state.setActivePage)
  const { data: scenarios = [] } = useQuery<SabotageScenarioDto[]>({ queryKey: ['sabotage', 'scenarios'], queryFn: listSabotageScenarios })
  const { data: modules = [] } = useQuery({ queryKey: ['modules', 'sabotage'], queryFn: listModules })
  const { data: events = [] } = useQuery({ queryKey: ['events', 'sabotage'], queryFn: listEvents })
  const { data: approvals = [] } = useQuery({ queryKey: ['approvals', 'sabotage'], queryFn: listApprovals })
  const [selectedScenarioId, setSelectedScenarioId] = useState<string>()
  const selectedScenario = scenarios.find((item) => item.key === selectedScenarioId) ?? scenarios[0]
  const selectedModule = modules.find((module) => module.key === selectedScenario?.moduleKey)
  const sabotageEvents = events.filter(isSabotageEvent)
  const sabotageApprovals = approvals.filter((approval) => sabotageEvents.some((event) => event.id === approval.eventId))
  const recentScenarioEvents = selectedScenario
    ? sabotageEvents.filter((event) => event.externalEventId?.includes(selectedScenario.key) || event.summary.includes(selectedScenario.summary.replace('[Sabotage] ', '')))
    : sabotageEvents
  const mutation = useMutation({
    mutationFn: (scenarioKey: string) => triggerSabotage(scenarioKey),
    onSuccess: (result) => {
      invalidateOperationalQueries(queryClient)
      pushToast({ tone: result.success ? 'success' : 'warning', title: result.message })
    },
    onError: (error) => pushToast({ tone: 'danger', title: 'Sabotage trigger failed', message: mutationErrorMessage(error) }),
  })
  const moduleReady = selectedScenario ? (!selectedScenario.requiresEnabledModule || selectedModule?.isEnabled) : false

  return (
    <PageFrame
      icon={AlertTriangle}
      title="Sabotage Lab"
      actions={<Button onClick={() => setActivePage('events')} type="button"><FileClock className="h-4 w-4" />Open Events</Button>}
    >
      <section className="sabotage-layout">
        <Panel className="sabotage-list-panel">
          <div className="module-section-head">
            <div>
              <h2 className="panel-title">Alert drills</h2>
              <div className="mt-1 text-xs text-slate-500">Controlled fault injections for local alert validation.</div>
            </div>
            <Badge tone={sabotageApprovals.some((item) => item.status === 'Pending') ? 'warning' : 'success'}>
              {sabotageApprovals.filter((item) => item.status === 'Pending').length} pending
            </Badge>
          </div>
          <div className="sabotage-scenario-list">
            {scenarios.map((scenario) => {
              const module = modules.find((item) => item.key === scenario.moduleKey)
              const isSelected = selectedScenario?.key === scenario.key
              return (
                <button
                  key={scenario.key}
                  className={cn('sabotage-scenario-card', isSelected && 'active')}
                  onClick={() => setSelectedScenarioId(scenario.key)}
                  type="button"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 text-xs text-slate-500">{module?.name ?? scenario.moduleKey} / {scenario.target}</div>
                    </div>
                    <span className={cn('action-enabled-dot', module?.isEnabled && 'on')} />
                  </div>
        ...
        <Panel className="sabotage-detail-panel">
          {selectedScenario ? (
            <>
              <div className="action-inspector-head">
                <div>
                  <div className="module-subtitle">{selectedModule?.name ?? selectedScenario.moduleKey}</div>
                  <h2 className="panel-title mt-1">{selectedScenario.name}</h2>
                </div>
                <SeverityBadge value={selectedScenario.severity} />
              </div>
              <p className="action-description">{selectedScenario.description}</p>
              <div className="action-facts">
                <DetailRow label="Event type" value={selectedScenario.eventType} />
                <DetailRow label="Target" value={selectedScenario.target} />
                <DetailRow label="Module state" value={selectedModule ? `${selectedModule.isEnabled ? 'Enabled' : 'Disabled'} / ${selectedModule.healthStatus}` : 'Module not found'} />
                <DetailRow label="Approval path" value={selectedScenario.likelyCreatesApproval ? 'Likely to create approval pressure' : 'Should remain operator-visible without approval'} />
              </div>
              <div className="action-policy-note">
                {selectedScenario.likelyCreatesApproval
                  ? 'This drill should create a real incident and, when the recommended remediation is high-risk, a real approval request in SafeMode.'
                  : 'This drill should create a real incident, diagnostics trail, and logs without filling the approval queue.'}
              </div>
              <div className="action-runbar">
                <Button disabled={!moduleReady || mutation.isPending} onClick={() => mutation.mutate(selectedScenario.key)} variant="primary">
                  {mutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <AlertTriangle className="h-4 w-4" />}
                  Trigger sabotage
                </Button>
              </div>
              {!moduleReady ? <div className="mt-3 text-sm text-amber-200">Enable the linked module first, then rerun the drill.</div> : null}
            </>
          ) : (
            <EmptyState icon={AlertTriangle} title="No sabotage scenarios available" />
          )}
        </Panel>
        <Panel className="sabotage-results-panel">
          <div className="module-section-head">
            <div>
              <h2 className="panel-title">Recent drill results</h2>
              <div className="mt-1 text-xs text-slate-500">Events and approvals created by sabotage scenarios.</div>
            </div>
            <Badge>{sabotageEvents.length} events</Badge>
          </div>
          <div className="sabotage-result-list">
            {recentScenarioEvents.slice(0, 8).map((event) => {
              const approval = sabotageApprovals.find((item) => item.eventId === event.id)
              return (
                <button key={event.id} className="timeline-item" onClick={() => openEvent(event.id)} type="button">
                  <span className="timeline-rail">
                    <span className={cn('timeline-dot', healthPulseClass(event.status === 'PendingApproval' ? 'Warning' : event.status === 'Completed' ? 'Healthy' : 'Error'))} />
                  </span>
                  <span className="min-w-0">
                    <strong className="block truncate text-sm text-stone-100">{event.summary}</strong>
                    <span className="mt-2 flex flex-wrap gap-2 text-xs text-stone-500">
                      <span className="mono-chip">{event.target}</span>
                      <span>{timeLabel(event.createdAt)}</span>
                      {approval ? <span className="event-state-chip">{approval.status}</span> : null}
                    </span>
                  </span>
            ...
            {recentScenarioEvents.length === 0 ? <EmptyState icon={AlertTriangle} title="No sabotage results yet" /> : null}
          </div>
          <div className="mt-4 flex justify-end">
            <Button onClick={() => setActivePage('approvals')} type="button">
