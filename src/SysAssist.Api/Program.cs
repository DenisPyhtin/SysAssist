...
if (builder.Configuration.GetValue("ReverseProxy:TrustForwardedHeaders", false))
{
...
var sabotage = api.MapGroup("/sabotage").RequireAuthorization("RequireEngineerOrHigher").WithTags("Sabotage");
sabotage.MapGet("/scenarios", async (ISysAssistApiService service, CancellationToken cancellationToken) =>
    Results.Ok(await service.ListSabotageScenariosAsync(cancellationToken)));
sabotage.MapPost("/{key}/trigger", async (string key, ClaimsPrincipal user, HttpContext context, ISysAssistApiService service, CancellationToken cancellationToken) =>
    Results.Ok(await service.TriggerSabotageAsync(key, Actor(user), CorrelationId(context), cancellationToken)))
    .RequireRateLimiting(WriteRateLimitPolicy);
