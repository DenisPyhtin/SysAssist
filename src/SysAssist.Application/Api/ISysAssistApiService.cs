...
public interface ISysAssistApiService
{
    ...
    Task<IReadOnlyCollection<SabotageScenarioDto>> ListSabotageScenariosAsync(CancellationToken cancellationToken);
    Task<OperationResultDto> TriggerSabotageAsync(string key, string actor, string? correlationId, CancellationToken cancellationToken);
